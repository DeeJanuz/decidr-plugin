...
- **decidr_audit_dashboard** — Audit event dashboard with filters and event drilldowns
- **decidr_audit_reports** — Audit report builder with saved reports, custom payload fields, sharing, version history, and CSV export
...
The dashboard, audit dashboard, audit reports, timeline, and graph headers expose a glassmorphism organization picker. Click a row to switch the active org for the current session; click the star to persist that org as your default across sessions (backed by `/api/me/preferences`). Orgs whose plugin token is not yet connected show a "Connect" badge and can still be starred as the default.
