...
2. **Use the selected work logging policy.** If a DecidR work logging startup rule is installed, follow it. `auto_log_confident` means clear low-risk DecidR writes may proceed directly after search/context checks; `review_first` means present formal writes for review before execution; `capture_only` means use Work Sessions for compact handoff and ask before formal DecidR logging.
...
- With `capture_only`, keep compact Work Sessions active and ask before creating, updating, promoting, or lifecycle-transitioning formal DecidR records.
